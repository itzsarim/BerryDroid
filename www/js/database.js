...
$(document).ready(function(){
    ...
    $('#listBtn').click(function(){
        newListName = $('input[name=newListName]').val();
        $('#list').append('<div class="item">' + newListName + '</div>' );
        $('input[name=newListName]').remove();
        $('#listBtn').remove();
    ...
    $('#itemBtn').click(function(){
        ...
        $('#list').append('<div class="item">' +itemID+'.'+newItemName + '</div>' );
        ...
        select(table_list,"*","list=?",[newListName],function(rows){
            if(rows){
                ...
                alert("total rows are "+rows.length);
                var lastItem = rows.length-1;
                alert("the item you added is "+rows.item(lastItem).item);
            }
        });
    });
});
